...
`;
        ...
        if (updatedFields && 'last_connected' in updatedFields) {
          const alertMessage = `
📢 Record Updated!
➡️ _id: ${change.documentKey._id}
➡️ Updated Fields: ${JSON.stringify(updatedFields, null, 2)}
                    `;
          
          console.log('Document updated:', updatedFields);
          
          if (!config.telegram.chatId) {
            console.error('Error: Telegram chat ID is undefined.');
            process.exit(1);
          }
